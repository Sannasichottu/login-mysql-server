            ...
            if(result.length<=0){
                ...
                if (!(await bcrypt.compare(password, result[0].PASS))) {
                    ...
                    const cookieOptions = {
                      ...
                      res.cookie("chottu", token, cookieOptions);
...
exports.register = (req,res) => {
  ...
  db.query('select email from users where email=?',[email], async(error,result) => {
     ...
     if(error){
        ...
        console.log(result)
        return res.render ('register',{msg:"User Registration Success 😍", msg_type :"goodc "})
    }
        }
        );
    }
